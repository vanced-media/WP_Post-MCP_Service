...
// ---------------------------------------------------------
// Tool 1: Get_PostList
// Uses WP Native API to easily fetch content and scan for the HTML comment flag
// ---------------------------------------------------------
server.tool(
    "Get_PostList",
    ...
    async ({ page = 1, per_page = 10 }) => {
        try {
            ...
            const posts = data.map(p => {
                ...
                // Scan for the hidden HTML comment we inject
...
// Uses Assist Agent API to fetch full post details
// ---------------------------------------------------------
server.tool(
    "Get_SinglePost",
...
// Uses Assist Agent API to update a post, and invisibly injects the edit timestamp
// ---------------------------------------------------------
server.tool(
    "Edit_SinglePost",
    ...
    async ({ id, title, content, status }) => {
        try {
            ...
            // If content is not provided, we must fetch the existing content to append the timestamp
            ...
            // Remove any existing mcp_edited comment to avoid duplicates
            finalContent = finalContent.replace(/<!-- mcp_edited:\s*\d+\s*-->\s*/g, '');
            
            // Append the new timestamp comment
            finalContent += `\n<!-- mcp_edited: ${Date.now()} -->`;

            const payload = {};
            if (title !== undefined) payload.title = title;
            if (status !== undefined) payload.status = status;
            payload.content = finalContent; // Always include content to inject the tag
